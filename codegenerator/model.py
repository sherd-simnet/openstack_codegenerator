...
class Reference(BaseModel):
    ...
    def __hash__(self):
        return hash((self.name, self.type_, self.hash_))
...
class JsonSchemaParser:
    ...
    def parse_object(
        ...
        if obj:
            ...
            if (
                obj.reference
                and f"{obj.reference.name}{obj.reference.type_}"
                in [
                    f"{x.reference.name}{x.reference.type_}"
                    for x in results
                    if x.reference
                ]
            ):
                if obj.reference.__hash__() in [
                    ...
                    if parent and name:
                        ...
                        if Reference(
                            name=new_name, type=obj.reference.type_
